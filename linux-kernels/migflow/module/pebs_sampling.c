...
#include "/home/koo/src/hmsdk/linux/kernel/events/internal.h"
		...
		__u64 type, __u32 pid)
{
	...
	if (config == ALL_STORES)
		...
		attr.sample_period = get_sample_period(9);
...
static int ksamplingd(void *data)
{
	...
	unsigned long sample_period = 10;
	...
	*/
	...
	while (!kthread_should_stop()) {
		...
		if ((cur - bw_cputime) >= bw_period) {
			...
			printk("read_period: %llu, write_period: %llu, nr_read_period: %llu, nr_write_period: %llu, nr_accesses_per_sec: %llu\n", read_period, write_period, nr_read_period, nr_write_period, nr_read_period * read_period + nr_write_period * write_period);
